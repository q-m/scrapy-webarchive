...
class WaczFileCreator:
    ...
    def write_datapackage(self, zip_file: zipfile.ZipFile):
        package_dict = {
            "profile": "data-package",
            "title": self.collection_name,
            "description": "This is the web archive generated by a Scrapy extension for the " \
                f"{self.collection_name} spider. It is mainly for scraping purposes as it does not contain " \
                "any js/css data. Though it can be replayed as bare HTML if the site does not depend on JS.",
            "created": get_formatted_dt_string(format=WARC_DT_FORMAT),
            "software": f"scrapy-webarchive v{scrapy_webarchive_version}, using Scrapy v{scrapy_version}",
            "resources": [{}],
        }
        ...
        for zip_entry in zip_file.infolist():
            with zip_file.open(zip_entry, "r") as stream:
                size, hash_ = hash_stream("sha256", stream)
        ...
        zip_file.writestr("datapackage.json", json.dumps(package_dict, indent=2))
